...
const JourneyCard = ({ item, index, smoothMode }) => {
  const isEven = index % 2 === 0;
  
  if (smoothMode) {
    return (
      <div className={`flex flex-col md:flex-row items-center justify-between mb-24 w-full relative ${isEven ? 'md:flex-row-reverse' : ''}`}>
...
const Journey = ({ data, smoothMode }) => {
  ...
  const height = smoothMode ? "100%" : scrollHeight;
  ...
  return (
    <div ref={containerRef} className="relative max-w-7xl mx-auto px-4 py-20 overflow-hidden">
      ...
      <div className="relative z-10 pt-10">
        {data.map((item, index) => (
          <JourneyCard key={index} item={item} index={index} smoothMode={smoothMode} />
        ))}
      </div>
